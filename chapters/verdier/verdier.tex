...
\begin{proof}
    ...
    \prepp{Is this good enough? Should probably say why we can safely swap.}
\end{proof}
\begin{lemma}
    ...
    \end{enumerate}
    Then any $F'\in \D$ can be restricted to a sheaf $\F \in \Shv(X;\C)$ and is given by the fiber $\Gamma_K(X;\F)$ when restricted to $M_0$.
\end{lemma}
\begin{proof}
    ...
    \]
    and by the limi-cofinality of $i$ we get
    \[
...
\end{proof}
\begin{corollary}
    In particular, $F\colon M \to \C$ satisfying condition $2.$ in \ref{5.5.5.7} is given by $\Gamma_K(X;\F)$ when restricted to $M_0$.
\end{corollary}
\begin{proof}
    Observe first that $M$ is a full subcategory of $M'$ and then that $\MAP(M \to \C)$ is a full subcategory of $\D$.
...
\begin{proof}
    ...
    By Proposition \ref{4.3.2.8} it is enough to show that $F$ is a left Kan extension of $F|_{M''}$, and this is enough to check at every $(2,S) \in M_2$.
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item $F'|_{M'' \cup B}$ is a left Kan extension of $F|_{M''}$
              ...
              Fix $U \in \Open(X)$ such that $\overline{U} \in \K(X)$. By \ref{KanDef} we want to show that $F'(2, X-U)$ is a colimit of the diagram $F'|_{M''/(2,X-U)}$.
              For $K \in \K(X)$ denote by $M_K''$ the subset of $M''$ consisting of pairs $(i,S)$ such that $(0,K)\leq (i,S) \leq (2,X-U)$.
              Now, observe that $M''_{/(2,X-U)}$ is a filtered colimit of $\{M_K''\}_{K\in \K(X)_{U/}}$.
...
\begin{proof}
    ...
    \TODO{Make sure this colimit is taken in the correct category. It should maybe just be a limit in $\C$?}
    \prepp{Should I write out theta more explicitly in the proof of \ref{7.3.4.9}. Not a lot of time.}
    ...
    By Theorem \ref{7.3.4.9} the restriction $\D\to \Fun(\Open(X)^{op}, \C)$ is a categorical equivalence onto $\Shv(X;\C)$, since we Kan extend along the fully faithful inclusion of $\Shv(X;\C)$ (it is a full subcategory of $\Fun(\Open(X)^{op}, \C)$.)
    \newline
