...
\begin{proposition}[{\cite[\href{https://kerodon.net/tag/004B}{Proposition 004B}]{kerodon}}]
    ...
    \end{enumerate}
\end{proposition}
\TODO{Place the following in the correct place: This means that $([h]\circ [g])\circ [f] = [w]$ and $[h] \circ ([g]\circ [f]) = [h]\circ [u]$, so it remains to show that $[w] = [h] \circ [u]$.}
\begin{proof}
...
\[\begin{tikzcd}
    ...
    \end{tikzcd}\]
\begin{definition}
    \TODO{The subcategory of a $\infty$-category}
\end{definition}
\begin{definition}
...
It can be shown that applying the same construction to $\infty$-categories $\C$ and $\D$ yields a new $\infty$-category $\MAP(\C, \D)$ with maps of simplicial sets as objects ($0$-cells) and natural transformations as morphisms ($1$-cells)\TODO{mention that these are maps $\C \times \Delta^1 \to \D$}.
...
\end{definition}
For a proof that this is always a space/$\infty$-groupoid/Kan complex, see \cite[Proposition 45.2]{Rezk}.
\begin{definition}
