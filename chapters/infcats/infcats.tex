...
\begin{document}
\TODO{I have used same notation for $1$-cats and $\infty$-cats. Consider changing.}
...
\subsection{Homotopy}
\TODO{Here $\Hom_{\C}(X,Y)$ means $\C_1$. Make this clear or change notation or something.}
...
\end{proposition}
\begin{proof}
    \TODO{Prove. Both Rune and Lurie have nice proofs. It is just drawing the correct horns and filling in.}
\end{proof}
\begin{proposition}
    $f,g \in \C_1$ are homotopic if and only if they are homotopic as morphisms in $\C^{op}$.
\end{proposition}
\TODO{See Run's comment.}
\begin{proof}
    \TODO{Draw horn.}
\end{proof}
...
\prepp{The proof seems kind of technical, so might not be worth doing. Might do it if there's time or Rune thinks I should.
    Considered adding \cite[\href{https://kerodon.net/tag/000Z}{Exercise 000Z}]{kerodon} as this exercise is used in a lot of the proofs in this subsection, but seems too technical for this text.}
...
See for example \TODO{reference Joyal2008 and mention ``certain stability properties of the class of categorical equivalences and the so-called inner anodyne maps'' or the proof of \cite{HTT}[1.2.7.3] on p.$94$.}
...
\end{definition}
\TODO{Maybe mention Rezk and weak saturated classes etc.}
