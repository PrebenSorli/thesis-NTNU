...
\begin{example}
    ...
    Letting $[0]$ denote the category with one object and one morphism, we denote by $\C^{\triangleleft}: = [0] \star \C$ the left cone of a $1$-category $\C$ and $\C^{\triangleright}:=\C \star [0]$ the right cone of $\C$.
...
\begin{definition}\label{joinsimpsets}
    Let $X, Y \in \sset$ and let $\Delta^+$ be the full subcategory of ordered sets obtained by adding the empty set $[-1]:=\emptyset$ to $\Delta$.
    We define the join $X\star Y$ on $n$-cells:
    \[
        (X\star Y)_n : = \coprod_{[n] = [n_1] \sqcup [n_2]} X_{n_1} \times Y_{n_2},
    \]
    where $[n_1], [n_2] \in \Delta_+$ and $\sqcup: \Delta_+ \times \Delta_+ \to \Delta_+$ is the ordered disjoint union.
