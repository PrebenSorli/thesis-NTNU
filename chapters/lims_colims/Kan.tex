...
In the final part of this thesis we will mostly consider the opposite special case in which $i$ is not the projection to a point, but rather the inclusion of a full subcategory.
...
Then, if $F$ is left Kan extended from $F_0$ there certainly is a natural transformation $F_0 \to F \circ i$ ``belonging'' to this Kan extension like we defined for $1$-categories, but we have chosen to omit it because it is an isomorphism in this case where we Kan extend along the inclusion of a full subcategory.
For Kan extensions along general functors, this natural transformation is no longer necessarily an isomorphism so it has to be part of the specified data.
...
\end{remark}
\begin{remark}
    This definition is compatible with the definition for Kan extensions along inclusions; just use the identity natural transformation as $\beta$.
\end{remark}
