...
\begin{definition}[{\cite[Definition 6.1.1.]{CatContext}}]
    ...
    \[\begin{tikzcd}
        ...
        \end{tikzcd}\]
    a left Kan extension of $F$ along $K$ is a functor $K_{!}F \colon \D \to \E$ together with a natural transformation $\eta \colon F \Rightarrow K_{!}F \circ K$ such that for any other pair $G \colon \D \Rightarrow \E, \gamma \colon F \to G \circ K$, $\gamma$ factors uniquely through $\eta$ as in the following diagram:
    \[\begin{tikzcd}
            \C && \E && \C && \E \\
            &&& {=} \\
            \D &&&& \D
...
Whenever $\C$ and $\D$ are respectively small and locally small, potential Kan extensions along $K\colon \C \to \D$ are what we call pointwise Kan extensions.
...
\end{corollary}
\begin{example}
    $n$-truncation is given by restricting along the inclusion $ i \colon \Delta_{\leq n} \hookrightarrow \Delta$, and since $\SET$ is both cocomplete and complete, we have both left and right Kan extensions to $n$-truncation.
    \[\begin{tikzcd}
            {\sset} && {\sset_{\leq n}}
...
\begin{example}\label{KanCone}
    ...
    However, as we discussed in the introduction of this section, Kan extending along the projection to a point is just the limit of $p$.
...
\begin{example}[Yoneda extension]
    ...
    By the corollary above, any functor $F\colon \C \to \E$ admits a left Kan extension $y_!F$ along the Yoneda embedding $y$ and since $y$ is fully faithful the unit $F \cong y_!F\circ y$ is an identity natural transformation.
...
\begin{proposition}[Transitivity of Kan extensions {\cite[Proposition 4.3.2.8]{HTT}}]\label{4.3.2.8}
    For a functor $F\colon \A\to\C$ of $\infty$-categories where $\A^0 \subseteq \A^1$ are full subcategories of $\A$, if $F|_{\A^1}$ is left Kan extended from $\A^0$, then $F$ is left Kan extended from $\A^1$ if and only if it is left Kan extended from $\A^0$.
\end{proposition}
...
\begin{definition}[{\cite[\href{https://kerodon.net/tag/02YC}{Variant 02YC}]{kerodon}}]
    Let $F \colon \C \to \D$ be a functor of $\infty$-categories.
    Suppose we are given a simplicial set $K$ together with functors $\delta \colon K \to \C$ and $F_0\colon K \to \D$ and a natural transformation $\beta \colon F_0 \to F \circ \delta$ like illustrated in the following diagram:
    \[\begin{tikzcd}
            K && \D \\
            \\
            \C
    ...
    \]
\end{definition}
\prepp{Lurie calls this an informal way of stating definition \cite[\href{https://kerodon.net/tag/02YC}{Variant 02YC}]{kerodon}, but his actual definition seems too technical so ok to just do this?}
