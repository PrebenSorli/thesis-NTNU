...
\begin{definition}[{\cite[Definition 6.2.2.1.]{HTT}}]
    ...
    \begin{enumerate}
        ...
        \item For an object $X \in \C$, a covering sieve $\C^{(0)}_{/X}$ on $X$, and an arbitrary  $\C^{(1)}_{/X}$ sieve on $X$, if the pullback $f^*\C^{(1)}_{/X}$ is a covering sieve on $Y$ for any $f\colon X \to Y$, then $\C^{(1)}_{/X}$ is a covering sieve on $X$.
    \end{enumerate}
\end{definition}
...
\begin{proof}
    ...
    Furthermore, we observe that a covering $\W$ is good if the open sets $\{K \cap W \mid W \in \W\}$ form a good covering for every $K\in \K(X)$.
    ...
    Now, we observe that we can use the theory of cocartesian fibrations to calculate the limit from $\Nerve(K_{\W}(X))^{op}$ as a limit of limits.


    Let $p \colon E \to P$ be a cocartesian fibration associated to the functor $P \to \mathrm{Cat}_{\infty}$ which sends $\alpha \in P$ to the $\infty$-category $\Nerve(\K_{\alpha})$.
    ...
    Moreover, for a cocartesian fibration, we know the map $E_{\alpha} \to E_{/\alpha}$ is right cofinal.
    ...
    \]
    and by right cofinality of $g$ we get
    \[
        \colim_{\alpha \in P}p_!\varphi \simeq \colim_{\alpha \in P}\colim_{\K_{\alpha}} \F = \colim_{\alpha \in P}\F(K),
    \]
    but we have already argued that $P$ is weakly contractible, so the map $P \to \Delta^1$ is right cofinal and we conclude that this colimit is also evaluated as $\F(K)$.
