...
\begin{proof}
    ...
    \]
    where $\K_{\W}(X)$ is the set $\{K\in \K(X) | (\exists W \in \W)[K \subseteq W] \}$.
    \TODO{Write out the details of why this is contractible and the inclusion is cofinal.}
    Observe that this cofinality implies that showing $(\F|(\W \cup \K_{\W}(X))^{op})$ is a right Kan extension of $\F|(\W \cup \K_{\W}(X) \cup \{U\})^{op}$ also shows
    \[
        \Nerve(\W)^{op} \hookrightarrow \Nerve(\W \cup \K_{\W}(X))^{op, \triangleleft}_{A/} \hookrightarrow \Nerve(\W \cup \K_{\W}(X) \cup \{U\})^{op, \triangleleft}_{A/} \to  \Nerve(\W \cup \K_{\W}(X) \cup \{U\})^{op} \xrightarrow{\F} \C
    \]
    is a limit diagram for every $A \in \W \cup \K_{\W}(X)$.
    ...
    \]
    \TODO{Rune has long comment about following argument.}
