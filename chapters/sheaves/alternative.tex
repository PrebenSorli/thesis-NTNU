...
\begin{proof}
    ...
    By \ref{superlemma} the inclusion is cofinal and we have shown that $\F_\K$ is a $\K$-sheaf.
\end{proof}
\begin{lemma}
...
\begin{proof}
    ...
    \]
    By Theorem \ref{superlemma} $f$ and $f'$ are cofinal inclusions if for any $Y\in (\Open(X) \cup \K(X))_{U/}$ the partially ordered sets
    \[
...
\begin{proof}
    ...
    \]
    and these are equivalences of $\infty$-categories because inclusions of full subcategories are fully faithful and Kan extensions along fully faithful functors give isomorphisms by Lemma \ref{KanFullyFaith}.
\end{proof}
\end{document}