...
\begin{theorem}[{\cite[Theorem 7.3.4.9]{HTT}}]\label{7.3.4.9}
    ...
    Let $\F \colon \Nerve(\Open(X) \cup \K(X))^{op} \to \C$.
...
\begin{proof}
    ...
    We will "transfer" this colimit to the colimit we want by right cofinal maps
    \[
    ...
    The simplicial set $\Nerve(\{K'' \in \K(X) \mid K \Subset K'' \subseteq K'\})$ is weakly contractible by exactly the same argument, and hence $p$ and $p'$ are right cofinal maps.
    By right cofinality of $p$ and $p'$, the diagram
    \[
...
\begin{proof}
    ...
    \]
    is a limit diagram.
    Let $\K_{\W}(X)$ is the set $\{K\in \K(X) | (\exists W \in \W)[K \subseteq W] \}$.
    ...
    \]
    if and only if, for any $Y \in \Nerve(\W \cup \K_{\W}(X))$ the simplicial set $\Nerve(\W)\times_{\Nerve(\W \cup \K_{\W}(X))}\Nerve(\W \cup \K_{\W}(X))_{Y/}$ is weakly contractible.
    This fiber product is equivalent to the nerve of the partially ordered set $\{W' \in \W | [\exists W \in \W][K \subseteq W \subseteq Y]\}$ and this is filtered, and hence, the fiber product is weakly contractible by \ref{5.3.1.20} and \ref{NerveFilter}.\footnote{\TODO{Check this calculation is correct with Tallak or Rune.}}
    Therefore it is enough to show the limit starting from $ \Nerve(\W \cup \K_{\W}(X))^{op}$.
    By example \ref{KanCone} this is equivalent to showing $\F|_{\Nerve(\W \cup \K_{\W}(X))^{op,\triangleleft}}$ is right Kan extended from $\F|_{\Nerve(\W \cup \K_{\W}(X))^{op}}$.
...
\begin{proof}
    ...
    By \ref{superlemma} this implies $\Nerve(P^{op}) \to \Nerve((\Open(X)_{/(K\cap K')})^{op})$ is right cofinal and we get a colimit diagram
    \[
    ...
    \]
    is a colimit diagram, and the statement is reduced to showing that $\Nerve(\K_{K\Subset}(X)) \subseteq \Nerve(\K(X) \cup \Open(X))_{/K}^{op}$ is right cofinal.
    Let $Y\in \Nerve(\K(X) \cup \Open(X))_{/K}$ and let $R$ be the partially ordered set $\{K' \in \K(X) \mid K \Subset K ' \subseteq Y\}$.
    Since $R$ is nonempty and stable under intersections, $R^{op}$ is filtered and hence $\Nerve(R)$ is weakly contractible.
    By \ref{superlemma} the inclusion is right cofinal and we have shown that $\F_\K$ is a $\K$-sheaf.
\end{proof}
\begin{lemma}
...
\begin{proof}
    ...
    \]
    By Theorem \ref{superlemma} $f$ and $f'$ are right cofinal inclusions if for any $Y\in (\Open(X) \cup \K(X))_{U/}$ the partially ordered sets
    \[
