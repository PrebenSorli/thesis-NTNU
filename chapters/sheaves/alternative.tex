...
\begin{theorem}[{\cite[][Theorem 7.3.4.9]{HTT}}]\label{7.3.4.9}
    ...
    \end{enumerate}
\end{theorem}
We will split the theorem into a few lemmas for readability.
\begin{lemma}
...
\end{proof}
\begin{lemma}
    If $\F_\K:= \F|\Nerve(\K(X))^{op}$ is a $\K$-sheaf, and $\F$ is a right Kan extension of $\F_\K$, then $\F_{\Open}$ is a sheaf.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{lemma}
    If $\F_{\Open}:=\F|\Nerve(\Open(X))^{op}$ is a sheaf, and $\F$ is a left Kan extension of $\F_{\Open}$, then $\F_{\K}$ is a $\K$-sheaf.
\end{lemma}
\begin{proof}
